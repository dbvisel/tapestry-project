...
function shouldProcessItemThumbnail(item: ItemWithAssets, patch?: ItemUpdateDto) {
  if (item.scheduledThumbnailProcessing) {
    return false
  }
...
async function processCustomThumbnailUpdate(
  item: Item,
  ...
  const items = await ensureTransaction(tx, (tx) =>
    Promise.all(
      Object.entries(updates).map(async ([id, update]) => {
        ...
        await processCustomThumbnailUpdate(dbItem, update.thumbnail, tx)
        ...
        if (shouldProcessItemThumbnail(updatedDbItem, update)) {
          if (updatedDbItem.thumbnail?.renditions.every((r) => r.isAutoGenerated)) {
            await tx.imageAsset.delete({ where: { id: updatedDbItem.thumbnail.id } })
          }
