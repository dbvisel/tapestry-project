...
If `VITE_AUTH_PROVIDER` is set to `mediawiki`, the Tapestries frontend will display a "Sign in with MediaWiki" button. The application uses [MediaWiki's OAuth 2.0](https://www.mediawiki.org/wiki/OAuth/For_Developers) authorization-code flow: the frontend redirects the user to the wiki, the wiki redirects back with an authorization code, and the backend exchanges that code for an access token and then fetches the authenticated user's profile (username, central user id, and — if confirmed and granted — email). Register an OAuth 2.0 consumer at `Special:OAuthConsumerRegistration/propose` on your wiki to obtain a client ID and secret, and set the callback URL to the Tapestries app origin.

Configure the backend with `MEDIAWIKI_CLIENT_ID` and `MEDIAWIKI_CLIENT_SECRET`, and the frontend with `VITE_MEDIAWIKI_CLIENT_ID`. `MEDIAWIKI_BASE_URL` / `VITE_MEDIAWIKI_BASE_URL` is the wiki's REST endpoint (default `https://meta.wikimedia.org/w/rest.php`). The OAuth callback defaults to the app's own origin but can be overridden with `MEDIAWIKI_REDIRECT_URI` / `VITE_MEDIAWIKI_REDIRECT_URI`. See [`MEDIAWIKI.md`](/MEDIAWIKI.md) for a full setup guide.
